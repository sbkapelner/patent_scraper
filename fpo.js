...
const createUrl = (input, page) => {
  const baseUrl = `https://www.freepatentsonline.com/result.html?p=${
    page + 1
  }&edit_alert=&srch=xprtsrch&query_txt=`;
  const encodedInput = encodeURIComponent(input);
  const encodedInputRegex = encodedInput.replace(/%20/g, "+");
  const searchParam =
    "&uspat=on&usapp=on&eupat=on&jp=on&pct=on&depat=on&date_range=all&stemming=on&sort=relevance&search=Search";
